...
function Feedback({ msg }) {
  if (!msg) return null;
  return (
    <p className={`cfg-feedback cfg-feedback--${msg.tipo}`}>{msg.texto}</p>
  );
}
...
function Configuracoes() {
  ...
  useEffect(() => {
    ...
    const local = (() => {
      try {
        return JSON.parse(localStorage.getItem("cn-config") || "null");
      } catch {
        return null;
      }
    })();
    ...
    get(ref(realtimedb, `users/${user.uid}/config`))
      .then((snap) => {
        if (cancelado) return;
        if (snap.exists()) {
  ...
  useEffect(() => {
    ...
    document.documentElement.classList.toggle(
      "cn-no-anim",
      !!config.aparencia.reduzirAnimacoes
    );
    update(ref(realtimedb, `users/${user.uid}/config`), config).catch(() => {});
    setSalvoFlag(true);
  ...
  async function salvarPerfil(e) {
    ...
    try {
      ...
      setMsgPerfil({
        tipo: "erro",
        texto: "Não foi possível salvar o perfil.",
      });
    } finally {
  ...
  async function alterarEmail(e) {
    ...
    try {
      ...
      setMsgConta({
        tipo: "ok",
        texto: "E-mail alterado! Talvez seja preciso entrar de novo.",
      });
      setNovoEmail("");
      setSenhaAtual("");
    } catch (err) {
  ...
  const setJogo = (id, v) =>
    setConfig((c) => ({ ...c, jogos: { ...c.jogos, [id]: v } }));
  const setNotif = (id, v) =>
    setConfig((c) => ({ ...c, notif: { ...c.notif, [id]: v } }));
  const setApar = (id, v) =>
    setConfig((c) => ({ ...c, aparencia: { ...c.aparencia, [id]: v } }));
  const setPriv = (id, v) =>
    setConfig((c) => ({ ...c, privacidade: { ...c.privacidade, [id]: v } }));
      ...
      <div className="cfg-shell">
        ...
        <div className="cfg-layout">
          ...
          <nav className="cfg-nav" aria-label="Seções de configurações">
            ...
            <button
              className="cfg-nav-item cfg-nav-item--sair"
          ...
          <div className="cfg-conteudo">
            ...
            {secao === "perfil" && (
              <section className="cfg-card">
                ...
                <p className="cfg-card-sub">
                  Como você aparece no site e na comunidade.
                </p>
                ...
                <div className="cfg-avatar-bloco">
                  {fotoURL ? (
                    ...
                    <span className="cfg-avatar cfg-avatar--inicial">
                      {getInicial(user)}
                    </span>
                  )}
                  <div className="cfg-avatar-info">
                    <span className="cfg-avatar-nome">
              ...
              <section className="cfg-card">
                ...
                {ehSenha && (
                  <>
                    <form className="cfg-form" onSubmit={alterarEmail}>
                      ...
                      />
                      <button
                        className="cfg-btn cfg-btn--secundario"
                        type="submit"
                      >
                        Alterar e-mail
                      </button>
                    ...
                    <form className="cfg-form" onSubmit={alterarSenha}>
                      ...
                      />
                      <span className="cfg-dica">
                        Você também precisará informar a senha atual no campo
                        acima.
                      </span>
                      <button
                        className="cfg-btn cfg-btn--secundario"
                ...
                <div className="cfg-perigo">
                  <div className="cfg-perigo-texto">
                    ...
                    <span>Remove permanentemente sua conta e dados.</span>
                  </div>
                  <button className="cfg-btn cfg-btn--perigo" onClick={excluirConta}>
                    Excluir conta
                  </button>
                </div>
              </section>
            ...
            {secao === "preferencias" && (
              <section className="cfg-card">
                ...
                <p className="cfg-card-sub">
                  Personalize o que você vê no Counter-News.
                </p>

                <h3 className="cfg-grupo-titulo">
                  Jogos que você acompanha (por enquanto, apenas o
                  Counter-Strike está disponível)
                </h3>
                ...
                <LinhaOpcao
                  titulo="Região preferida"
                  descricao="Prioriza notícias e ranking dessa região."
                >
                  <Dropdown
                    options={[
                    ...
                    value={config.regiao}
                    onChange={(v) => setConfig((c) => ({ ...c, regiao: v }))}
                ...
                <LinhaOpcao
                  titulo="Idioma"
                  descricao="Idioma de exibição do conteúdo."
                >
                  <Dropdown
                    options={[
                    ...
                    value={config.idioma}
                    onChange={(v) => setConfig((c) => ({ ...c, idioma: v }))}
                  />
                </LinhaOpcao>

                <LinhaOpcao
                  titulo="Modo sem spoiler"
                  descricao="Oculta placares e resultados de partidas até você clicar."
                >
                  <Toggle
                    checked={config.semSpoiler}
                    onChange={(v) =>
                      setConfig((c) => ({ ...c, semSpoiler: v }))
                    }
                  />
                </LinhaOpcao>
              </section>
            ...
            {secao === "notificacoes" && (
              <section className="cfg-card">
                ...
                <LinhaOpcao
                  titulo="Newsletter por e-mail"
                  descricao="Resumo semanal das principais notícias."
                >
                  <Toggle
                    checked={config.notif.newsletter}
                    onChange={(v) => setNotif("newsletter", v)}
                  />
                </LinhaOpcao>
                <LinhaOpcao
                  titulo="Últimas horas"
                  descricao="Avisos de notícias de última hora."
                >
                  <Toggle
                    checked={config.notif.ultimaHora}
                    onChange={(v) => setNotif("ultimaHora", v)}
                  />
                </LinhaOpcao>
                <LinhaOpcao
                  titulo="Partidas dos favoritos"
                  descricao="Quando seus times favoritos forem jogar."
                >
                  <Toggle
                    checked={config.notif.partidas}
                    onChange={(v) => setNotif("partidas", v)}
                  />
                </LinhaOpcao>
                <LinhaOpcao
                  titulo="Atualizações dos jogos"
                  descricao="Patches e novidades dos jogos que você acompanha."
                >
                  <Toggle
                    checked={config.notif.atualizacoes}
                    onChange={(v) => setNotif("atualizacoes", v)}
                  />
                </LinhaOpcao>
                <LinhaOpcao
                  titulo="Respostas na comunidade"
                  descricao="Quando alguém responder você no chat."
                >
                  <Toggle
                    checked={config.notif.comunidade}
                    onChange={(v) => setNotif("comunidade", v)}
                  />
                </LinhaOpcao>
              </section>
            )}
            ...
            {secao === "aparencia" && (
              <section className="cfg-card">
                ...
                <p className="cfg-card-sub">
                  Ajuste a experiência visual do site.
                </p>

                <LinhaOpcao
                  titulo="Tema"
                  descricao="O tema escuro é o padrão do Counter-News."
                >
                  <Dropdown
                    options={[{ value: 'escuro', label: 'Escuro' }]}
                    value="escuro"
                    onChange={() => {}}
                    disabled
                  />
                ...
                <LinhaOpcao
                  titulo="Densidade"
                  descricao="Espaçamento entre os elementos."
                >
                  <Dropdown
                    options={[
                    ...
                    value={config.aparencia.densidade}
                    onChange={(v) => setApar("densidade", v)}
                  />
                </LinhaOpcao>
                <LinhaOpcao
                  titulo="Reduzir animações"
                  descricao="Diminui transições e efeitos de movimento."
                >
                  <Toggle
                    checked={config.aparencia.reduzirAnimacoes}
                    onChange={(v) => setApar("reduzirAnimacoes", v)}
                  />
                </LinhaOpcao>
              </section>
            )}
            ...
            {secao === "privacidade" && (
              <section className="cfg-card">
                ...
                <p className="cfg-card-sub">
                  Controle como você aparece para os outros.
                </p>

                <LinhaOpcao
                  titulo="Perfil público na comunidade"
                  descricao="Permite que outros vejam seu nome e avatar no chat."
                >
                  <Toggle
                    checked={config.privacidade.perfilPublico}
                    onChange={(v) => setPriv("perfilPublico", v)}
                  />
                </LinhaOpcao>
                <LinhaOpcao
                  titulo="Mostrar status online"
                  descricao="Exibe quando você está online na comunidade."
                >
                  <Toggle
                    checked={config.privacidade.statusOnline}
                    onChange={(v) => setPriv("statusOnline", v)}
                  />
                </LinhaOpcao>
              </section>
            )}
            ...
            {salvoFlag && (
              <span className="cfg-salvo">
                <Check size={15} aria-hidden="true" /> Preferências salvas
              </span>
            )}
          </div>
        </div>
