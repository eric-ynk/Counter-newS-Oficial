...
const ptComponents = {
  types: {
    image: ({ value }) => {
      ...
      return <img src={value.imageUrl} alt={value.alt || ''} className='noticia-img-corpo' />;
    }
  }
};
  ...
  return (
    <article className='noticia-page'>
      <div className='noticia-hero'>
        ...
        {post.category && <span className='noticia-categoria-pill'>{post.category}</span>}
      </div>
        ...
        <div className='noticia-conteudo'>
          ...
          <div className='noticia-meta'>
            ...
            <span className='noticia-tempo'>
              <Clock size={14} aria-hidden="true" /> {tempoLeitura} min de leitura
            </span>
          </div>
          ...
          {relacionadas.length > 0 && (
            <section>
              ...
              <div className='relacionadas-lista'>
                {relacionadas.map((r) => (
                  <Link to={`/noticia/${r.slug}`} key={r._id} className='relacionada-card'>
                    <img src={r.imageUrl} alt={r.title} />
                    <div>
                      <p className='relacionada-titulo'>{r.title}</p>
                      <span className='relacionada-credito'>{r.authorCredit}</span>
                    </div>
                  </Link>
                ))}
              </div>
